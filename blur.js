...
function draw(image, clippingObj){
    ...
    context.drawImage(image, Math.max(leftMargin,0), Math.max(topMargin,0), Math.min(canvas.width, image.width), Math.min(canvas.height, image.height), theLeft, theTop, Math.min(canvas.width, image.width), Math.min(canvas.height, image.height));
    context.restore();
}
...
showBtn.addEventListener("click", function(){
    var animate_show = setInterval(function(){
        clipObj.r += 50;
        draw(img, clipObj);
        if(clipObj.r > Math.sqrt(Math.pow(canvas.width,2) + Math.pow(canvas.height,2))){
